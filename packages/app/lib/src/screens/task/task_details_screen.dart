                ...
                Row(
                  children: [
                    if ((inProgress || inCompleted) && task.startedAt != null) ...[
                      TaskInfo(
                        title: context.appLocalization.taskStartedOn,
                        value: "${task.startedAt!.getTime(context)} ",
                      ),
                    ],
                    if (inCompleted && task.endedAt != null) ...[
                      const SizedBox(width: 10),
                      TaskInfo(title: context.appLocalization.dueAt, value: task.endedAt!.getTime(context)),
                    ],
                    if (inProgress && (task.endTime?.isAfter(kNowDate) ?? false)) ...[
                      ...
                      TimerBuilder(
                        ...
                        child: (time) {
                          return TaskInfo(title: context.appLocalization.mustBeCompletedWithin, value: time);
                        },
                      ),
                    ],
                  ],
